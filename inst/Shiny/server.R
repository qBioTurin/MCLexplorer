...
server <- function(input, output, session) {
  ...
  observe({
    ...
    isolate({
      ...
      df <- merge(df, CONNECTORList.FCM$CONNECTORList$LabCurv %>% dplyr::select(ID, IDold), by = "ID")
  ...
  output$MCL_clusteringPlot <- renderPlot({
    ...
    completeDF <- merge(df, Dataset %>% dplyr::select(-Cluster), by = "ID")
  ...
  output$MCL_survPlot <- renderPlot({
    ...
    if (selectSurvMCL == "Arm - single cluster") {
      ...
      ggsurvList <- lapply(c(as.list(unique(Dataset$Cluster)), list(c("A", "B"), c("C", "D"))), function(cl, Dataset) {
        ...
        if (!all(is.na(surv_median_values$median))) {
          ggsurv$plot <- ggsurv$plot +
            annotate("text",
              x = surv_median_values$median, y = 0,
      ...
      ggsurv$plot <- ggsurv$plot +
        ...
        scale_x_continuous(
          breaks = seq(0, max(ggsurv$table$data$time), 24),
    ...
    if (!is.null(MCLvalues$ClusterDF)) {
      ...
      if (dim(df_sub)[1] > 0) {
        plot <- ggplot(df_sub) +
          ...
          scale_x_continuous(
            breaks = seq(0, max(df_sub$Time), 90),
  ...
  output$MCL_FittedCurve <- renderPlot({
    ...
    MCLvalues$splinePlots[[ID]] +
      scale_x_continuous(
        breaks = seq(0, max(grid), 60),
        labels = seq(0, max(grid), 60) / 30
      ) +
      labs(x = "Months", y = "") +
      scale_y_continuous(
        limits = c(min(-1, low), max(hig, 8)),
        breaks = seq(0, 8, 2),
        labels = c("NEG", "POS", TeX("$10^{-3}$"), TeX("$10^{-2}$"), TeX("$10^{-1}$"))
      )
  })
  ...
  output$DTtableMCL <- DT::renderDT({
    ...
    if (arm == "NULL") {
      pz_data <- info_pz_MCL0208_connector %>%
        ...
        dplyr::select(-Cluster_BM, -Cluster_PB)
    } else {
      dftmp <- df %>%
        dplyr::select(-ID) %>%
        mutate(ID = paste0(IDold)) %>%
        dplyr::select(ID, Cluster) %>%
        distinct()
      pz_data <- merge(info_pz_MCL0208_connector %>% dplyr::select(-Cluster_BM, -Cluster_PB), dftmp)
    }
    ...
    DT::datatable(pz_data[, c("ID", "Cluster", vars_values)],
      filter = "top",
  ...
  output$MCL_ClinicalPlot <- renderPlot({
    ...
    if (variable %in% qual_vars) {
      ...
      if (input$ClusterCheckMCL) {
        ...
        plot <- test$plot +
          labs(caption = paste(
            "Shapiro test for normality: * = pvalue<0.05, ** = p-value<0.01 \n",
  ...
  listV <- reactiveValues(
    CONNECTORList.FCM = list(),
  ...
  observeEvent(input$tissueBox, {
    ...
    df = listV$classificationFCM
    df$Step<-as.factor(df$Step)
    df$Step<-recode_factor(df$Step,
                           "between EOI and ASCT" = "Ara-C",
                           "cycle3"="EOI",
                           "cycle4"="EOI",
                           "cycle6"="EOI"
    )
    ...
    DisTimeBX <- df %>%
      ...
      geom_label(
        data = df %>% group_by(Step) %>% summarise(Time = median(Time / 365)),
  ...
  observe({
    ...
    Dataset <- WholeData$YoungMCL[[input$tissueBox]]$Dataset  %>% dplyr::select(-Cluster)
    ...
    output$classYoungPlot <- renderPlot({
      pl <- ggplot(DatasetClusters, aes(x = Time, y = Observation, group = ID)) +
        ...
        scale_x_continuous(
          breaks = seq(0, max(DatasetClusters$Time), 60),
    ...
    ggsurvplot <- ggplot() +
      ...
      geom_point(
        data = ggsurv$plot$data[ggsurv$plot$data$n.censor > 0, , drop = FALSE],
        aes(x = time, y = surv, color = strata), size = 4, shape = "+"
      ) +
      theme_bw() +
      labs(x = "Years", y = "TTP", fill = "Cluster", color = "Cluster") +
      scale_color_manual(values = colors) +
      scale_fill_manual(values = colors) +
      scale_x_continuous(breaks = c(0, 3, 6, 9, 12, 15) * 365.25, labels = c(0, 3, 6, 9, 12, 15)) +
      annotate("text",
        x = pval$pval.x, y = pval$pval.y,
    ...
    output$survYoungPlot <- renderPlot({
      ggsurvplot / ggsurv$table + plot_layout(heights = c(3, 1))
    })
    ...
    YmclSummary <- paste(
      ...
      paste(names(table(classification$Classification$ClassMatrix_entropy$Cluster)), ": ",
        table(classification$Classification$ClassMatrix_entropy$Cluster),
        collapse = ",\n"
      ),
      ...
      output$linePlot <- renderPlot({
        if (!is.null(Data)) {
          ...
          pl <- (pl +
            labs(x = "Months", y = "") +
            scale_x_continuous(
              breaks = seq(0, max(Data$Time), 60),
              labels = seq(0, max(Data$Time), 60) / 30
            )) / listV$DisTimeBX + plot_layout(heights = c(2, 0.5))
          return(pl)
        }
      })
    ...
    isolate({
      ...
      if (!is.null(listV$ClassificationOUT) && !is.null(listV$ClassificationOUT$ggsurv)) {
        ...
        ggsurvplot <- ggplot(data = ggsurv$plot$data, aes(x = CutTime + time)) +
          ...
          theme_bw() +
          labs(x = "Year", y = "TTP", color = "Cluster", fill = "Cluster") +
          scale_color_manual(values = colors) +
          scale_fill_manual(values = colors) +
          scale_x_continuous(breaks = c(0, 3, 6, 9, 12, 15) * 365.25, labels = c(0, 3, 6, 9, 12, 15)) +
          annotate("text",
        ...
        if (!all(is.na(surv_median_values$median))) {
          ggsurvplot <- ggsurvplot +
            ...
            annotate("text",
              x = CutTime + surv_median_values$median, y = 0,
        ...
        pl1 <- pl1 +
          ...
          labs(x = "Months", y = "") +
          scale_x_continuous(
            breaks = seq(0, max(meancurves$Time), 120),
            labels = seq(0, max(meancurves$Time), 120) / 30
          )+
          scale_y_continuous(
            limits = c(-1, 8),
            breaks = seq(0, 8, 2),
  ...
  observe({
    ...
    if (is.null(ListValuesUsers$ClassificationINIT)) {
      ...
      Info_tmp <- DatasetClusters %>%
        dplyr::select(-Time, -Observation) %>%
        distinct()
      ...
      ListValuesUsers$ClassificationINIT <- list(
        ggsurvData = ggsurv$data.survplot,
        Classification = ClassMatrix_entropy
      )

      output$survUserPlot <- renderPlot({
        ggsurv
      })
    } else {
      DatasetClusters <- merge(ListValuesUsers$merged_df,
    ...
    UserSummary <- paste(
      ...
      paste(names(table(ListValuesUsers$ClassificationINIT$Classification$Cluster)), ": ",
        table(ListValuesUsers$ClassificationINIT$Classification$Cluster),
        collapse = ",\n"
      ),
  ...
  observe({
    ...
    isolate({
      ...
      if (!is.null(ListValuesUsers$DataFCMtrunc) && dim(ListValuesUsers$DataFCMtrunc)[1] > 0) {
        ...
        Info_tmp <- merge(ListValuesUsers$DataFCMtrunc, ClassMatrix_entropy) %>%
          dplyr::select(-Time, -Observation) %>%
          distinct()
        ...
        ggsurvplot <- ggplot() +
          ...
          theme_bw() +
          labs(x = "Year", fill = "Cluster", color = "Cluster") +
          scale_color_manual(values = colors) +
          scale_fill_manual(values = colors) +
          annotate("text",
            x = pval$pval.x, y = pval$pval.y,
  ...
  observeEvent(list(input$saveLP_fromClass_user, input$saveLP_user), {
    if (!is.null(ListValuesUsers$LPanalysis)) {
      lapply(seq_along(ListValuesUsers$LPanalysis), function(i) {
        ...
        output[[paste("plot_user", i, sep = "")]] <- renderPlot({
          p
        })
      })
    }
